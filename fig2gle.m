function fig2gle(pOutputFilename, hFigure, varargin)

% fig2gle(pOutputFilename,hFigure,varargin);
%
%
%
% Khan 12/16/2010
...
% and titles.
%
% fig2gle(pOutputFilename,FigureHanle,varagin)
%
% pOutputFilename     is a string with the file name without directory
...
%              script generates a directory named pOutputFilename in the
%              current directory to save all the CSV data files,
%              the GLE file, and the PDF file (if requested).
%
% hFigure is the handle of the figure you wish to generate a
%              GLE file for.
%
...
if ~exist(pOutputFilename,'dir')
    mkdir(pOutputFilename);
else
    rmdir(pOutputFilename,'s');
    mkdir(pOutputFilename);
end
...
figure(hFigure);
if (OCTAVE_FLAG ~= 0)
    f = get(hFigure,'Children');
    Ngraphs = length(f);
    % Octave plot handles seem to be always negative
    f = sort(f,'descend');
else
    % Legend handles
    L = findobj(hFigure,'Type','axes','Tag','legend');
    L = sort(L);
    % Axes handles
    f = findobj(hFigure,'Type','axes','Tag','');
    Ngraphs = length(f);
    f = sort(f);
end
...
GLE_File = sprintf('%s/%s.gle',pOutputFilename,pOutputFilename); 
fid = fopen(GLE_File,'wt');
paper_position = get(hFigure,'PaperPosition');
...
for j = 1:Ngraphs
	...
	DatFileName = [pOutputFilename '/' CSV_FileName];
	M = [x_data' y_data'];
...
if defined('PDF_Flag') && (PDF_Flag == 1)
