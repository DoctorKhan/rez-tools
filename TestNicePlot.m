...
%% Load Data
vLaminaPts = [-0.75, -0.2500, 0.1650, 0.5000, 0.8350, 1.2500, 1.75];

load /export/mri/LaminarHemo/031010cg/omBootParamsV1c0p6074u50Long.mat

a = ForEachFieldOf(omBootParams, @CalcStats);

ydata_m = a.FWHMValue.mean;
xdata_m = vLaminaPts;
ydata_u = a.FWHMValue.upper - a.FWHMValue.mean;
ydata_l = a.FWHMValue.mean - a.FWHMValue.lower;

xmodel = xdata_m - min(xdata_m) + max(xdata_m(:)) + 1;
ymodel = ydata_m;
ymodelL = ydata_l;
ymodelU = ydata_u;
...
hFit   = line(xfit  , yfit   );
hE     = errorbar(xdata_m, ydata_m, ydata_l, ydata_u);
