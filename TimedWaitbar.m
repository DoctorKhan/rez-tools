function progf= TimedWaitbar(varargin)

% progf= TimedWaitbar(varargin);
%
% USAGE:
% TimedWaitbar(prog)                  % Update progress bar.
% TimedWaitbar(text)                  % New message text.
% TimedWaitbar(prog, text)            % Update progress bar and new message text.
% TimedWaitbar(text, arg1, arg2, ...) % New message text, works like sprintf(...).
% TimedWaitbar()                      % Deletes the TimedWaitbar window.
%
% And for compatibility with the original Matlab TimedWaitbar:
% TimedWaitbar(prog, handle)
% handle= TimedWaitbar(prog, handle, text)
% handle= TimedWaitbar(prog, text, <property name/value list>)
%
% A Matlab TimedWaitbar compatible replacement that is faster and more functional.
%
% ARGUMENTS:
...
% 4.  Only one TimedWaitbar window, so no old ones left around.
% 5.  Remaining time estimated and presented.
% 6.  Delay of 5 seconds before the TimedWaitbar is shown (no flashing at quick tasks).
...
% nEnd= 100;
% TimedWaitbar(0, '(1 of 2) Preparing...', 'DelayPeriod', 0, 'LingerPeriod', 0, ...
%                 'MinUpdateTime', 0, 'Name', 'Other title...', 'BarColor', [0 1 0]);
% for n= 1:nEnd
% pause(0.1); % Prepare stuff
% TimedWaitbar(n/nEnd);
% end
% TimedWaitbar(0, '(2 of 2) Calculating...');
% for n= 1:nEnd
...
% - Mixed calls to plot-like functions and TimedWaitbar now works better.
...
% - Mixing calls to plot-like functions and TimedWaitbar should now work. Many thanks to Yair Altman,
%   who suggested fixes.
% - Fixed a minor cosmetic bug (placement of bar).
% Version 1.5, 2006-09-12.
% - Prefixes percentage to window title, e.g. '29% Progress'.
% - Now the TimedWaitbar window is put on top of all figures after every call to it.
...
if(~isempty(win) && (abs(thistime(6)-win.last) <= win.mintime))
    if((nargin == 1) && isnumeric(varargin{1}) && (varargin{1} < 1))
        return;		% Calling TimedWaitbar(prog).
    elseif((nargin == 2) && isnumeric(varargin{1}) && (varargin{1} < 1) && isnumeric(varargin{2}))
        return;		% Calling TimedWaitbar(prog, handle).
    end
end
...
%%% A call to TimedWaitbar() hides the progress window.	%%%%% No arguments: Hide window!
if(nargin == 0)
    db=			dbstack;
    progf=		[];
    if(length(db) < 2),	db(2).name= '';	end			% Calling from the command win.
    try
        if(isempty(win.start) || isempty(findstr(db(2).name, 'TimedWaitbar')))
            % We want to reset this, in case we actually finished a process and then
            % call TimedWaitbar without a message (previous one would show). If we close
            % the TimedWaitbar window in midprocess, the message text should not be deleted.
            win.msg=			'';
...
%%% Check if we have a TimedWaitbar window already, create a new one if there is none.
try
    if(~win.text)
...
%%% We have a TimedWaitbar and we want to do something with it.
prog= 			varargin{1};						% Progress value.
msgstr=			0;									% Default value (no message).
if(isnumeric(prog) && (numel(prog) == 1))			%%%%% Update TimedWaitbar.
    ...
    if((nargin == 3) && isnumeric(varargin{2}) && ischar(varargin{3}))
        msgstr=		varargin{3};		% TimedWaitbar(prog, handle, message).
    elseif((nargin == 2) && (isnumeric(varargin{2}) || isempty(varargin{2})))
        % Do nothing, but let through.	% TimedWaitbar(prog, handle).
    elseif((nargin >= 2) && ischar(varargin{2}) && (2*floor(nargin/2) == nargin))
        msgstr=		varargin{2};		% TimedWaitbar(prog, message, <optional name/value list>).
        if(~win.text && (nargin > 2) && ~strcmp(varargin{3}, 'CreateCancelBtn'))
            win=		set_properties(win, prog, varargin{3:end});
        end
    elseif(nargin >= 2)
        error('Wrong input argument(s), see ''help TimedWaitbar'' for correct use. [A]');
    end
    ...
    elseif(prog >= 1)
        ...
        % Wait to remove TimedWaitbar in case another (sub)task follows directly.
...
elseif(ischar(prog))								%%%%% New message.
    progf=		TimedWaitbar(0, sprintf(varargin{:}));	% Default progress in this case is 0.
    return;
else
    error('Wrong input argument(s), see ''help TimedWaitbar'' for correct use. [B]');
end
...
%%% Put the TimedWaitbar window on top of all others.
% 	children=		allchild(0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	Create default TimedWaitbar (gui or text?).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(1 == prod(get(0,'ScreenSize')))				%%%%% Textual interface.
    ...
    % Calculate position of TimedWaitbar window and bar.
    ...
    % Delete all pre-existing TimedWaitbar graphical objects.
