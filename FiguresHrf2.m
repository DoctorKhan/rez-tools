function [mhPlot, mhLegend, vhFig, voLoad] = FiguresHrf2(xROI, iTypeDef, oHemo, viBins, vScale, iPolyFit)
	...
	switch(iType)
		case {1,2}
			if defined('voFit')
				...
				% Smin and max y values
				iDim = ~(MatchDims(mY, vL)-1)+1;
				mWindow = [Smin(mY, iDim) - vL, Smax(mY,iDim) + vU];
				...
				try voLabels(4).title = oROI.comments; end
				...
				for oParam = voPlotData
					...
					for iFitOrder = 1 %:iMaxOrder
                        ...
                        fPar = oParam.vFitParams(iFitOrder);
                        thick = 1;
						sM = RoundSig(fPar/thick, 2, 'string');
						sText = Str(sText, sM, sPm, sP);
					end
					
					[vhPlot(iParam), vhLegend(iParam)] = PlotWithError(oParam, voBins, voLabels(iParam), oParam.hPlot, iType);
